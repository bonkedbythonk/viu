...
First off, thank you for considering contributing to Anicat! We welcome any help, whether it's reporting a bug, proposing a feature, or writing code. This document will guide you through the process.

## How Can I Contribute?

There are many ways to contribute to the Anicat project:

*   **Reporting Bugs:** If you find a bug, please create an issue in our [issue tracker](https://github.com/anicat-media/Anicat/issues).
...
*   **Adding a Provider, Player, or Selector:** Extend Anicat's capabilities by integrating new tools and services.

## Contribution Workflow

We follow the standard GitHub Fork & Pull Request workflow.

1.  **Create an Issue:** Before starting work on a new feature or a significant bug fix, please [create an issue](https://github.com/anicat-media/Anicat/issues/new/choose) to discuss your idea. This allows us to give feedback and prevent duplicate work. For small bugs or documentation typos, you can skip this step.

2.  **Fork the Repository:** Create your own fork of the Anicat repository.

3.  **Clone Your Fork:**
    ```bash
    git clone https://github.com/YOUR_USERNAME/Anicat.git
    cd Anicat
    ```
...
9.  **Submit a Pull Request:** Open a pull request from your branch to the `master` branch of the main Anicat repository. Provide a clear title and description of your changes.
...
*   **Modularity and Architecture:**
    ...
    *   **Configuration:** All configuration is managed through Pydantic models in `anicat/core/config/model.py`. When adding new config options, update the model, defaults, and descriptions.
...
1.  **Create a New Provider Directory:** Inside `anicat/libs/provider/anime/`, create a new directory with the provider's name (e.g., `anicat/libs/provider/anime/newprovider/`).
...
2.  **Implement the Provider:**
    ...
    *   Create `mappers.py` to convert the provider's data structures into the generic types defined in `anicat/libs/provider/anime/types.py`.
...
3.  **Register the Provider:**
    *   Add your new provider to the `ProviderName` enum in `anicat/libs/provider/anime/types.py`.
    *   Register it in the `PROVIDERS_AVAILABLE` dictionary in `anicat/libs/provider/anime/provider.py`.

4.  **Add Normalization Rules (Optional):** If the provider uses different anime titles than AniList, add mappings to `anicat/assets/normalizer.json`.

## How to Add a New Player

1.  **Create a New Player Directory:** Inside `anicat/libs/player/`, create a directory for your player (e.g., `anicat/libs/player/myplayer/`).
...
3.  **Add Configuration (if needed):**
    *   If your player has configurable options, add a new Pydantic model (e.g., `MyPlayerConfig`) in `anicat/core/config/model.py`. It should inherit from `OtherConfig`.
...
4.  **Register the Player:**
    *   Add your player's name to the `PLAYERS` list in `anicat/libs/player/player.py`.
    *   Add the logic to instantiate your player class within the `PlayerFactory.create` method.

## How to Add a New Selector

1.  **Create a New Selector Directory:** Inside `anicat/libs/selectors/`, create a new directory (e.g., `anicat/libs/selectors/myselector/`).
...
4.  **Register the Selector:**
    *   Add your selector's name to the `SELECTORS` list in `anicat/libs/selectors/selector.py`.
    *   Add the instantiation logic to the `SelectorFactory.create` method.
    *   Update the `Literal` type hint for the `selector` field in `GeneralConfig` (`anicat/core/config/model.py`).

## How to Add a New CLI Command or Service

Our CLI uses `click` and a `LazyGroup` class to load commands on demand.

### Adding a Top-Level Command (e.g., `anicat my-command`)

1.  **Create the Command File:** Create a new Python file in `anicat/cli/commands/` (e.g., `my_command.py`). This file should contain your `click.command()` function.

2.  **Register the Command:** In `anicat/cli/cli.py`, add your command to the `commands` dictionary.
    ```python
    commands = {
        # ... existing commands
...
### Adding a Subcommand (e.g., `anicat anilist my-subcommand`)

1.  **Create the Command File:** Place your new command file inside the appropriate subdirectory, for example, `anicat/cli/commands/anilist/commands/my_subcommand.py`.

2.  **Register the Subcommand:** In the parent command's entry point file (e.g., `anicat/cli/commands/anilist/cmd.py`), add your subcommand to the `commands` dictionary within the `LazyGroup`.
    ```python
    @click.group(
        cls=LazyGroup,
        # ... other options
...
### Creating a Service
If your command involves complex logic, consider creating a service in `anicat/cli/service/` to keep the business logic separate from the command-line interface. This service can then be instantiated and used within your `click` command function. This follows the existing pattern for services like `DownloadService` and `PlayerService`.

---
Thank you for contributing to Anicat
