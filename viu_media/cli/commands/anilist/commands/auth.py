...
def auth(
    ...
    \b
    1. --token flag:     viu anilist auth --token <token>
    2. Positional arg:   viu anilist auth <token_or_path>
    3. Env variable:     $ANILIST_TOKEN
    4. Token file:       ~/.config/viu/token.txt
...
def _handle_status(auth_service, feedback):
    ...
    if user_data:
        ...
        feedback.error(
            "Not logged in.",
            "Run 'viu anilist auth --token <token>' or set $ANILIST_TOKEN to authenticate.",
        )
...
def _show_auth_instructions(feedback):
    ...
    feedback.info(
        ...
        "  • viu anilist auth --token <token>\n"
        "  • export ANILIST_TOKEN='<token>'\n"
        "  • Save token to ~/.config/viu/token.txt\n"
        "  • Add 'token = \"<token>\"' under [anilist] in config.toml\n"
        "\n"
        "Then run 'viu anilist auth' again to complete authentication."
    )
